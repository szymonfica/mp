...
(define-type Exp
  ...
  (varE [x : Symbol])
  (letE [xs : (Listof (Symbol * Exp))] [e : Exp])
  (let*E [xs : (Listof (Symbol * Exp))] [e : Exp]))
...
(module+ test
  ...
  (test/exn (parse `{^ 1 2})
            "unknown operator")
  (test (parse `{let ((x 1) (y 2)) {+ x y}})
        (letE (list (pair 'x (numE 1)) (pair 'y (numE 2))) (opE (add) (varE 'x) (varE 'y)))))
    ...
    [(let*E xs e)
      (type-case (Listof (Symbol * Exp)) xs
        ...
        [(cons y ys)
         (eval
          (let*E ys e) 
          (extend-env env (fst y) (eval (snd y) env)))])]))

(define (eval-let [xs : (Listof (Symbol * Exp))] [env : Env]) : Env
  (type-case (Listof (Symbol * Exp)) xs
    [empty env]
    [(cons y ys)
       (extend-env (eval-let ys env) (fst y) (eval (snd y) env))]))

; (run `(let ([x 1] [y 3]) (+ y x)))
; (run `(let* ([x 1]  [y (+ x 1)]) (+ y x)))
...
(module+ test
  ...
  (test (run `{let ((x 1))
                {+ x {let ((x {+ x 1}))
                       {* x 3}}}})
        (numV 7))
  (test (run `(let ([x 1] [y 3]) (+ y x)))
        (numV 4))
  (test (run `(let* ([x 1]  [y (+ x 1)]) (+ y x)))
        (numV 3)))

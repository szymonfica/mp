...
(define (gen [reset : Boolean]) : Void
  (if reset
      (set! ctr 0)
      (set! ctr (add1 ctr))))

(define (new-var) : Symbol
  (string->symbol (string-append bans (to-string (begin (gen #f) ctr)))))

(define (reset-var) : Void
  (gen #t))



(define (rev-translate [a : ExpA] [env : (EnvA Symbol)]) : Exp
  (type-case ExpA a
    [(numA n)
     (numE n)]
    [(opA o l r)
     (opE o (rev-translate l env) (rev-translate r env))]
    [(ifA b l r)
     (ifE (rev-translate b env)
          (rev-translate l env)
          (rev-translate r env))]
    [(varA n)
    ...
    [(letA a1 a2)
     (let ([s1 (new-var)])
       (letE s1
             (rev-translate a1 env)
             (rev-translate a2 (extend-envA env s1))))]))
    

(define (run-rev [s : S-Exp]) : Value
  (eval (rev-translate (translate (parse s) mt-envA) mt-envA) mt-env))
...
(module+ test
  ...
  (test (run `{= 0 1})
        (run-rev `{= 0 1}))
  (test (run `{if {= 0 1} {* 3 4} 8})
        (run-rev `{if {= 0 1} {* 3 4} 8}))
  (test (run `{let x 1 {+ x 1}})
        (run-rev `{let x 1 {+ x 1}}))
  (test (run `{let x 1 {+ x {let y 2 {* x y}}}})
        (run-rev `{let x 1 {+ x {let y 2 {* x y}}}}))
  (test (run `{let x 1
                {+ x {let x {+ x 1}
                       {* x 3}}}})
        (run-rev `{let x 1

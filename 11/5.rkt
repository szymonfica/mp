...
(define (find-sym? [sym : Symbol] [ys : (Listof Symbol)]) : Boolean
  (type-case (Listof Symbol) ys
    [empty #f]
    [(cons x xs)
     (cond
       [(eq? sym x) #t]
       [else (find-sym? sym xs)])]))

(define (fv-help [e : Exp] [xs : (Listof Symbol)]) : (Listof Symbol)
    (type-case Exp e
      [(numE n) empty]
      [(opE o l r) (append (fv-help l xs) (fv-help r xs))]
      [(ifE b l r) (append (fv-help b xs) (append (fv-help l xs) (fv-help r xs)))]
      [(varE x) (if (find-sym? x xs) empty (list x))]
      [(letE x e1 e2)
       (append (fv-help e1 xs) (fv-help e2 (cons x xs)))]
      [(lamE x e) (fv-help e (cons x xs))]
      [(appE e1 e2) (append (fv-help e1 xs) (fv-help e2 xs))]))

...
(define (decompile-help [xs : (Listof Instr)] ys)
  (cond
    [(empty? xs) ys]
    [(pushI? (first xs)) (decompile-help (rest xs) (cons (numE (pushI-n (first xs))) ys))]
    [(opI? (first xs)) (decompile-help (rest xs) (cons (opE (opI-op (first xs)) (second ys) (first ys)) (rest (rest ys))))]
    ))


(define (decompile [xs : (Listof Instr)]) : Exp
  (first (decompile-help xs empty))
                   )
(module+ test
  ...
  (test (decompile (compile (parse `(+ 1 (+ 2 3)))))
        (parse `(+ 1 (+ 2 3))))
  (test (decompile (compile (parse `(+ (* 1 (- 2 1)) (+ 2 3)))))
        (parse `(+ (* 1 (- 2 1)) (+ 2 3))))
  )
